...
We can import information about each of the mock taxa for implementing pipeline 2 step 4.
```{r}
mock_taxa = read.csv('mockTaxa.csv', row.names = 1)
mock_taxa %>% kable()
```

### Known contaminants

As this data is from an even mock dataset (Hulspusch et al., 2023), we are able to see only the expected sequences from the undiluted mock microbial sample (D0).
```{r}
expected_D0 = read.csv('expectedD0.csv', row.names = 1)
expected_D0 %>% kable()
```

We can use each of these to see how the methods are performing.
...
```{r}
blocklist = c('Actinomyces','Corynebacterium','Arthrobacter',
...
$$FL(J) = 1 - \frac{\|X^T_{-J} X_{-J}\|_F^2}{\|X^TX\|F^2}$$
