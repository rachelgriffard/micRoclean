...
Next, we are going to load our count and metadata matrix.
...
As this data is from an even mock dataset (Hulspusch et al., 2023), we have the known contaminants available to us.

```{r}
mock_taxa = read.csv('mockTaxa.csv')
mock_taxa %>% kable()
```
